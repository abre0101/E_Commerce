  ...
  return (
    <div className="space-y-5 max-w-6xl">
      ...
      <div className="grid grid-cols-3 gap-5">
        {/* List */}
        <div className="col-span-2 bg-white rounded-2xl border overflow-hidden" style={{ borderColor: "#e5e7eb" }}>
          <div className="px-5 py-4 border-b flex gap-2" style={{ borderColor: "#f3f4f6" }}>
          ...
          <div className="divide-y" style={{ borderColor: "#f3f4f6" }}>
            {filtered.map(item => {
              ...
              return (
                <div key={item.id} onClick={() => setSelected(item)} className="p-4 cursor-pointer transition-all"
                  style={{ background: isActive ? "rgba(22,163,74,0.04)" : "transparent", borderLeft: isActive ? "3px solid #16a34a" : "3px solid transparent" }}>
                  <div className="flex items-start justify-between mb-1.5">
                    <div>
                      <p className="font-semibold text-sm" style={{ color: "#111827" }}>{item.name}</p>
                      <p className="text-xs" style={{ color: "#6b7280" }}>{item.email}</p>
                    </div>
                    <span className="px-2 py-0.5 rounded-full text-xs font-semibold" style={ps}>{item.priority}</span>
                  </div>
                  <p className="text-sm font-semibold mb-0.5" style={{ color: "#111827" }}>{item.subject}</p>
                  <p className="text-xs line-clamp-1 mb-2" style={{ color: "#6b7280" }}>{item.message}</p>
            ...
            {filtered.length === 0 && <div className="py-12 text-center text-sm" style={{ color: "#6b7280" }}>No inquiries found.</div>}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border" style={{ borderColor: "#e5e7eb" }}>
          {selected ? (
            <div className="flex flex-col h-full">
              <div className="p-5 border-b" style={{ borderColor: "#f3f4f6" }}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-sm" style={{ color: "#111827" }}>{selected.name}</p>
              ...
              <div className="p-5 flex-1 space-y-4 overflow-y-auto">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wider mb-1" style={{ color: "#6b7280" }}>Subject</p>
                  <p className="font-semibold text-sm" style={{ color: "#111827" }}>{selected.subject}</p>
                </div>
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wider mb-1" style={{ color: "#6b7280" }}>Message</p>
                  <p className="text-sm p-3 rounded-xl" style={{ color: "#111827", background: "#f9fafb" }}>{selected.message}</p>
                </div>
                <div>
                ...
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wider mb-2" style={{ color: "#6b7280" }}>Reply</p>
              ...
              <div className="p-5 pt-0 flex gap-2">
                <button onClick={handleReply} className="flex-1 py-2.5 rounded-xl font-semibold text-white text-sm"
                  style={{ background: "linear-gradient(135deg,#15803d,#22c55e)" }}>Send Reply</button>
            ...
            <div className="flex flex-col items-center justify-center h-full py-16 px-6 text-center">
              ...
              <p className="text-sm font-semibold" style={{ color: "#111827" }}>No inquiry selected</p>
              <p className="text-xs mt-1" style={{ color: "#6b7280" }}>Click an inquiry to view and respond</p>
            </div>
          )}
        </div>
      </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
            ...
            <p className="font-bold text-lg mb-2" style={{ color: "#111827" }}>Delete Inquiry?</p>
            <p className="text-sm mb-6" style={{ color: "#4b5563" }}>Remove inquiry from <span className="font-semibold" style={{ color: "#111827" }}>{deleteTarget.name}</span>? This cannot be undone.</p>
