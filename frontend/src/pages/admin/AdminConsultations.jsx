  ...
  return (
    <div className="space-y-5 max-w-6xl">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm" style={{ color: "#9c8fa0" }}>{consultations.length} total appointments</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border overflow-hidden" style={{ borderColor: "#ede9f0" }}>
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map(c => {
              const s = SS(c.status);
              return (
                <tr key={c.id} className="border-t hover:bg-purple-50/30 transition-colors" style={{ borderColor: "#f3f0f6" }}>
                  <td className="px-5 py-3.5">
                    <p className="font-semibold" style={{ color: "#1a1825" }}>{c.name}</p>
                    <p className="text-xs mt-0.5" style={{ color: "#9c8fa0" }}>{c.email}</p>
                  </td>
                  <td className="px-5 py-3.5 text-sm" style={{ color: "#6b6070" }}>{c.type}</td>
                  <td className="px-5 py-3.5">
                    <p className="text-sm" style={{ color: "#1a1825" }}>{c.date}</p>
                    <p className="text-xs" style={{ color: "#9c8fa0" }}>{c.time}</p>
                  </td>
                  <td className="px-5 py-3.5">
        ...
        {filtered.length === 0 && <div className="py-12 text-center text-sm" style={{ color: "#9c8fa0" }}>No appointments found.</div>}
      </div>
        ...
        <Modal onClose={() => setShowAdd(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="px-6 py-5 border-b flex items-center justify-between" style={{ borderColor: "#f3f0f6" }}>
              <p className="font-bold text-lg" style={{ color: "#1a1825" }}>New Appointment</p>
              <button onClick={() => setShowAdd(false)} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-gray-100 text-lg font-bold" style={{ color: "#9c8fa0" }}>×</button>
            </div>
            <div className="p-6 space-y-3">
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={handleAdd} className="flex-1 py-2.5 rounded-xl font-semibold text-white" style={{ background: "linear-gradient(135deg,#f472b6,#a855f7)" }}>Save</button>
                <button onClick={() => setShowAdd(false)} className="flex-1 py-2.5 rounded-xl font-semibold" style={{ background: "rgba(168,85,247,0.08)", color: "#a855f7" }}>Cancel</button>
              </div>
            </div>
          </div>
        </Modal>
      ...
      {viewItem && (
        <Modal onClose={() => setViewItem(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="px-6 py-5 border-b flex items-center justify-between" style={{ borderColor: "#f3f0f6" }}>
              <p className="font-bold text-lg" style={{ color: "#1a1825" }}>Appointment Details</p>
              <button onClick={() => setViewItem(null)} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-gray-100 text-lg font-bold" style={{ color: "#9c8fa0" }}>×</button>
            </div>
            <div className="p-6 grid grid-cols-2 gap-4">
              {[["Name", viewItem.name], ["Phone", viewItem.phone], ["Email", viewItem.email], ["Type", viewItem.type], ["Date", viewItem.date], ["Time", viewItem.time]].map(([l, v]) => (
                <div key={l}>
                  <p className="text-[10px] font-bold uppercase tracking-wider mb-1" style={{ color: "#9c8fa0" }}>{l}</p>
                  <p className="text-sm font-semibold" style={{ color: "#1a1825" }}>{v || "—"}</p>
                </div>
              ))}
            ...
            <div className="px-6 pb-6 flex gap-3">
              <button onClick={() => setViewItem(null)} className="flex-1 py-2.5 rounded-xl font-semibold text-white" style={{ background: "linear-gradient(135deg,#f472b6,#a855f7)" }}>Close</button>
      ...
      {deleteTarget && (
        <Modal onClose={() => setDeleteTarget(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
            ...
            <p className="font-bold text-lg mb-2" style={{ color: "#1a1825" }}>Delete Appointment?</p>
            <p className="text-sm mb-6" style={{ color: "#6b6070" }}>This will permanently remove <span className="font-semibold" style={{ color: "#1a1825" }}>{deleteTarget.name}</span>'s appointment.</p>
