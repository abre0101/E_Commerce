...
const statusStyle = s => s === "VIP"
  ...
  : { bg: "rgba(156,143,160,0.15)", color: "#6b6070" };
...
export default function AdminCustomers() {
  ...
  const inputCls = "w-full px-3 py-2 rounded-xl text-sm border outline-none focus:ring-2 focus:ring-purple-200 transition-all";
  ...
  return (
    <div className="space-y-5 max-w-6xl">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Total", value: statCounts.total, color: "#a855f7", bg: "rgba(168,85,247,0.08)" },
          { label: "Active", value: statCounts.active, color: "#059669", bg: "rgba(52,211,153,0.08)" },
          { label: "VIP", value: statCounts.vip, color: "#f472b6", bg: "rgba(244,114,182,0.08)" },
        ].map(s => (
          <div key={s.label} className="rounded-2xl p-5 bg-white border" style={{ borderColor: "#ede9f0" }}>
            ...
            <p className="text-xs font-semibold uppercase tracking-wider mt-0.5" style={{ color: "#9c8fa0" }}>{s.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-2xl border overflow-hidden" style={{ borderColor: "#ede9f0" }}>
        {/* Toolbar */}
        <div className="flex flex-wrap items-center gap-3 px-5 py-4 border-b" style={{ borderColor: "#f3f0f6" }}>
            ...
            {filtered.map(c => {
              ...
              return (
                <tr key={c.id} className="border-t hover:bg-purple-50/30 transition-colors" style={{ borderColor: "#f3f0f6" }}>
                  ...
                  <td className="px-5 py-3.5 font-medium" style={{ color: "#6b6070" }}>{c.totalOrders}</td>
                  <td className="px-5 py-3.5 font-bold" style={{ color: "#a855f7" }}>{c.totalSpent}</td>
                  <td className="px-5 py-3.5 text-xs" style={{ color: "#9c8fa0" }}>{c.lastOrder}</td>
                  <td className="px-5 py-3.5">
        ...
        {filtered.length === 0 && (
          <div className="py-14 text-center" style={{ color: "#9c8fa0" }}>No customers found.</div>
        )}
      </div>
      ...
      {selected && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="p-6 border-b flex items-center justify-between" style={{ borderColor: "#f3f0f6" }}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold" style={{ color: "#1a1825" }}>{selected.name}</p>
                  <p className="text-xs" style={{ color: "#9c8fa0" }}>{selected.email}</p>
                </div>
              </div>
              <button onClick={() => setSelected(null)} className="w-8 h-8 rounded-full flex items-center justify-center text-lg font-bold hover:bg-gray-100" style={{ color: "#9c8fa0" }}>×</button>
            </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
            ...
            <p className="font-bold text-lg mb-2" style={{ color: "#1a1825" }}>Delete Customer?</p>
            <p className="text-sm mb-6" style={{ color: "#6b6070" }}>Are you sure you want to remove <span className="font-semibold" style={{ color: "#1a1825" }}>{deleteTarget.name}</span>? This cannot be undone.</p>
