...
const NAV = [
  { group: "Overview", items: [{ to: "/admin", label: "Dashboard", exact: true, icon: "dashboard" }] },
...
const ADMIN_GROUP = { group: "Team", items: [{ to: "/admin/staff", label: "Staff", icon: "staff" }] };

const PAGE_TITLES = {
  "/admin": "Dashboard", "/admin/orders": "Orders", "/admin/products": "Products",
  "/admin/consultations": "Consultations", "/admin/customers": "Customers",
  "/admin/inventory": "Stock", "/admin/promotions": "Promotions",
  "/admin/content": "Site Content", "/admin/inquiries": "Inquiries",
  "/admin/reports": "Reports", "/admin/staff": "Staff",
};
  ...
  return (
    <Link
      ...
      style={{
        ...
        color: active ? "#C9A961" : "#999",
        borderLeft: active ? "2px solid #C9A961" : "2px solid transparent",
      }}
      onMouseEnter={e => { if (!active) { e.currentTarget.style.color = "#C9A961"; e.currentTarget.style.background = "rgba(201,169,97,0.08)"; } }}
      onMouseLeave={e => { if (!active) { e.currentTarget.style.color = "#999"; e.currentTarget.style.background = "transparent"; } }}
    >
      <span style={{ color: active ? "#C9A961" : "#666", flexShrink: 0 }}>
        <Icon d={I[iconKey]} />
      </span>
...
export default function AdminLayout() {
  ...
  const SidebarContent = ({ isMobile = false }) => (
    <>
      {/* Logo */}
      <div className="flex items-center h-[60px] shrink-0"
        style={{
          padding: "0 16px",
        ...
        {(isMobile || !collapsed) && (
          <Link to="/" className="flex items-center gap-1 min-w-0">
            <span className="font-serif font-bold text-xl" style={{ color: "#fff" }}>Yada</span>
            <span className="font-serif font-bold text-xl" style={{ color: "#C9A961" }}>Hair</span>
            <span className="text-[10px] ml-2 px-1.5 py-0.5 font-bold uppercase tracking-wider"
              style={{ background: "rgba(201,169,97,0.15)", color: "#C9A961" }}>Admin</span>
          </Link>
        )}
        {isMobile ? (
          <button onClick={() => setMobileOpen(false)}
            className="w-7 h-7 flex items-center justify-center ml-auto" style={{ color: "#666" }}>
        ...
        {allGroups.map(({ group, items }, gi) => (
          <div key={group} style={{ marginBottom: gi < allGroups.length - 1 ? 16 : 0 }}>
            ...
            {collapsed && !isMobile && gi > 0 && (
              <div className="mx-auto mb-2 mt-1" style={{ width: 24, height: 1, background: "#222" }} />
            )}
            <div className="space-y-0.5">
              {items.map(item => (
                <NavItem key={item.to} to={item.to} iconKey={item.icon} label={item.label}
                  active={is(item.to, item.exact)} collapsed={collapsed && !isMobile} />
              ))}
            </div>
          </div>
        ))}
      ...
      {/* Footer */}
      <div style={{ borderTop: "1px solid rgba(201,169,97,0.15)", padding: "8px" }}>
        <div className="flex items-center mt-1"
          ...
          <div className="w-7 h-7 shrink-0 flex items-center justify-center text-black text-xs font-bold"
          ...
          {(!collapsed || isMobile) && (
            <div className="flex-1 min-w-0 flex items-center justify-between">
              <div className="min-w-0">
                <p className="text-xs font-semibold truncate leading-none" style={{ color: "#fff" }}>{session.name}</p>
                <p className="text-[10px] mt-0.5 capitalize" style={{ color: "#888" }}>{session.role}</p>
              </div>
              <button onClick={logout} title="Sign out"
                className="p-1 ml-1 shrink-0 transition-all" style={{ color: "#555" }}
  ...
  return (
    <div className="flex min-h-screen" style={{ background: "#0d0d0d" }}>
      ...
      {/* Mobile sidebar */}
      <aside className="fixed inset-y-0 left-0 z-50 flex flex-col transition-transform duration-300 lg:hidden"
        style={{
      ...
      <aside className="hidden lg:flex flex-col shrink-0 transition-all duration-300"
        style={{
          width: collapsed ? 64 : 240, background: "#111",
          borderRight: "1px solid rgba(201,169,97,0.15)",
        }}>
        <SidebarContent />
      </aside>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <header className="h-[60px] shrink-0 flex items-center justify-between"
          style={{ padding: "0 20px", background: "#111", borderBottom: "1px solid rgba(201,169,97,0.15)" }}>
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 text-sm" style={{ color: "#666" }}>
              <span className="hidden sm:inline">YadaHair</span>
              <span className="hidden sm:inline" style={{ color: "#444" }}>›</span>
              <span className="font-semibold" style={{ color: "#C9A961" }}>{pageTitle}</span>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="text-right hidden sm:block">
                <p className="text-xs font-semibold leading-none" style={{ color: "#fff" }}>{session.name}</p>
                <p className="text-[10px] mt-0.5 capitalize" style={{ color: "#888" }}>{session.role}</p>
              </div>
              <div className="w-8 h-8 flex items-center justify-center text-black text-sm font-bold"
                style={{ background: "#C9A961" }}>
                {session.name[0].toUpperCase()}
              </div>
            </div>
        ...
        {/* Content */}
        <main className="flex-1 overflow-auto" style={{ padding: "20px", background: "#0d0d0d" }}>
          <Outlet />
        </main>
      </div>
    </div>
