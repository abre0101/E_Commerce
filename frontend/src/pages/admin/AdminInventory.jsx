...
const stockStatus = item => {
  if (item.stock === 0) return { label: "Out of Stock", bg: "rgba(107,96,112,0.15)", color: "#4b5563" };
  ...
  return (
    <div className="space-y-5 max-w-5xl">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Products", value: inventory.length, color: "#16a34a", bg: "rgba(22,163,74,0.08)" },
      ...
      <div className="bg-white rounded-2xl border overflow-hidden" style={{ borderColor: "#e5e7eb" }}>
        <div className="px-5 py-4 border-b flex items-center justify-between" style={{ borderColor: "#f3f4f6" }}>
          <p className="font-bold" style={{ color: "#111827" }}>All Products</p>
          {low > 0 && (
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {inventory.map(item => {
              ...
              return (
                <tr key={item.id} className="border-t hover:bg-gray-50 transition-colors" style={{ borderColor: "#f3f4f6" }}>
                  <td className="px-5 py-3.5 font-semibold" style={{ color: "#111827" }}>{item.name}</td>
                  <td className="px-5 py-3.5 font-mono text-xs" style={{ color: "#6b7280" }}>{item.sku}</td>
                  <td className="px-5 py-3.5">
                    <span className="font-bold" style={{ color: "#111827" }}>{item.stock}</span>
                    <span className="text-xs ml-1.5" style={{ color: "#6b7280" }}>/ min {item.reorderLevel}</span>
                  </td>
                  <td className="px-5 py-3.5 font-semibold" style={{ color: "#16a34a" }}>{item.price}</td>
                  <td className="px-5 py-3.5">
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
            <div className="px-6 py-5 border-b flex items-center justify-between" style={{ borderColor: "#f3f4f6" }}>
              <p className="font-bold text-lg" style={{ color: "#111827" }}>Add Stock</p>
              <button onClick={() => setTarget(null)} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-gray-100 text-lg font-bold" style={{ color: "#6b7280" }}>×</button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-wider mb-1" style={{ color: "#6b7280" }}>Product</p>
                <p className="font-semibold" style={{ color: "#111827" }}>{target.name}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold uppercase tracking-wider mb-1" style={{ color: "#6b7280" }}>Current Stock</p>
                <p className="font-semibold" style={{ color: "#111827" }}>{target.stock} units</p>
              </div>
              <div>
                <label className="text-[10px] font-bold uppercase tracking-wider mb-2 block" style={{ color: "#6b7280" }}>Quantity to Add</label>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={() => setTarget(null)} className="flex-1 py-2.5 rounded-xl font-semibold" style={{ background: "rgba(22,163,74,0.08)", color: "#16a34a" }}>Cancel</button>
              </div>
            </div>
          </div>
        </div>
